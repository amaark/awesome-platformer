...
func _ready() -> void:
	...
	# This prevents the ledge climb ray cast from detecting the player's own collision shape
...
func switch_state(to_state: STATE) -> void:
	...
	# Set state specific things here that only need to run once upon entering the next state.
	match active_state:
		STATE.FALL:
			# Don't stop the double jump animation until we hit the ground
		...
		STATE.LEDGE_CLIMB:
			...
			# Align player's y position with the ledge
...
func process_state(delta: float) -> void:
	match active_state:
		STATE.FALL:
			...
			elif is_input_toward_facing() and is_ledge() and is_space_on_ledge():
		...
		STATE.JUMP, STATE.DOUBLE_JUMP, STATE.LEDGE_JUMP, STATE.WALL_JUMP:
			...
			if Input.is_action_just_released("jump") or velocity.y >= 0:
				velocity.y = 0
				switch_state(STATE.FALL)
			# Allow double jumping after a wall climb into an auto ledge jump
			# This is the only case where a jump input may be read during a jump state
			elif Input.is_action_just_pressed("jump"):
				switch_state(STATE.DOUBLE_JUMP)
			elif Input.is_action_just_pressed("sprint") and can_dash:
				switch_state(STATE.DASH)
		...
		STATE.FLOAT:
			...
			if is_on_floor():
				...
				switch_state(STATE.FALL)
			elif is_ledge() and is_space_on_ledge() and is_input_toward_facing():
				switch_state(STATE.LEDGE_CLIMB)
			elif can_wall_slide() and is_input_toward_facing():
				switch_state(STATE.WALL_SLIDE)
			elif Input.is_action_just_pressed("sprint") and can_dash:
				switch_state(STATE.DASH)
		...
		STATE.LEDGE_CLIMB:
			...
			if not animated_sprite.is_playing():
				...
				# This tells us the percentage of the ledge climb animation that has passed so far
		...
		STATE.WALL_SLIDE:
			...
			if is_on_floor():
				switch_state(STATE.FLOOR)
			elif is_ledge() and is_space_on_ledge():
				switch_state(STATE.LEDGE_CLIMB)
		...
		STATE.DASH:
			...
			var distance := absf(saved_position.x - position.x)
...
## Flip sprite, collision shape, and raycasts
func set_facing_direction(direction: float) -> void:
	if direction:
		animated_sprite.flip_h = direction < 0
		facing_direction = direction
		
		player_collider.position.x = direction * absf(player_collider.position.x)
		...
		# Prevent getting outdated collision information, call this whenever you move a raycast
...
## Are we touching a climbable ledge?
func is_ledge() -> bool:
	return is_on_wall_only() and \
	ledge_climb_ray_cast.is_colliding() and \
	# Ledge must have a horizontal surface to climb onto
	ledge_climb_ray_cast.get_collision_normal().is_equal_approx(Vector2.UP)

## Is there space atop the ledge for the player?
func is_space_on_ledge() -> bool:
...
## Snap player onto the ledge
func ledge_climb_offset() -> Vector2:
	var shape := player_collider.shape
	if shape is CapsuleShape2D:
		return Vector2(shape.radius * 2.0 * facing_direction, -shape.height * 0.5)
	if shape is RectangleShape2D:
		return Vector2(shape.size.x * facing_direction, -shape.size.y * 0.5)
	return Vector2.ZERO
